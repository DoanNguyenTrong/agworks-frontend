...
export const getPaymentCalculations = (
  orderId: string
): PaymentCalculation[] => {
  // Filter tasks for the specified work order
  const orderTasks = workerTasks.filter(
    (task) => task.orderId === orderId && task.status === "approved"
  );
  ...
  workerMap.forEach((data, workerId) => {
    ...
    const estimatedHoursPerTask = workOrder?.expectedHours
      ? workOrder.expectedHours / workOrder.neededWorkers
      : 4;
